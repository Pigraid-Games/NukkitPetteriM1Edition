...
public class BedrockBatchWrapper extends AbstractReferenceCounted {
    private static final Recycler<BedrockBatchWrapper> RECYCLER =
            new Recycler<BedrockBatchWrapper>() {
                @Override
    ...
    private final Recycler.Handle<BedrockBatchWrapper> handle;

    private ByteBuf compressed;
    private PacketCompressionAlgorithm algorithm;
    ...
    public void setCompressed(ByteBuf compressed, PacketCompressionAlgorithm algorithm) {
        if (this.compressed != null) {
            this.compressed.release();
        }
